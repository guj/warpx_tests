...
while ! grep -q "Total" "$OUTPUT"; do
	...
	echo "==============="
	    ...
	    OP_NETWORK_MPIDP="--network=single_node_vni,job_vni"
            echo "  calling reorg: srun --nodes=1 --ntasks-per-node=${NUM_RANKS} ${OP_NETWORK_MPIDP} --exclusive ${ADIOS_REORG}   ${SSTDIR}/${result} ${result}.bp5 SST [${SST_OPTION}] BPFile [${OUT_OPTION}] [${RANK_LOAD}]"	    
            #srun -N 1 ${ADIOS_REORG}  ${SSTDIR}/${result}  ${result}.bp5 SST "verbose=5" BPFile ""  &
	    srun --nodes=1 --ntasks-per-node=${NUM_RANKS} ${OP_NETWORK_MPIDP} --exclusive ${ADIOS_REORG}  ${SSTDIR}/${result}  ${SSTDIR}/${result}.bp5 SST "${SST_OPTION}" BPFile "${OUT_OPTION}" ${RANK_LOAD} > out.${result}.reader 2>&1 &
