...
#SBATCH -N 5
# #SBATCH -o %x-%j.out
#SBATCH -o log.rdma1.o%j
#SBATCH -e log.rdma1.e%j
...
runMe()
{
    ...
    srun -N${SIM_NNODES} -n${TOTAL_NMPI} --ntasks-per-node=${WARPX_NMPI_PER_NODE} --exclusive ${EXE} ${INPUTS}  > ${OUTPUT_DIR}/output.${key}_${aggDesc} 2>&1 &
    echo "now monitor ${MONITOR}  ${OUTPUT_DIR}/output.${key}_${aggDesc} diags/diag1 .sst ${SST_TRANSPORT} 1 1"
    pwd
    bash  ${MONITOR}  ${OUTPUT_DIR}/output.${key}_${aggDesc} diags/diag1 .sst ${SST_TRANSPORT} 1 "1"
